class Cyclops {
    ...
    update() {
        ...
        if (!this.attacking) {
            let target = this.adventurer; //target will be player
    ...
    draw(ctx) {
        if (this.dead) {
            ...
            this.animations[this.facing][this.state].drawFrame(this.game.clockTick, ctx, this.x - this.game.camera.x, this.y - this.game.camera.y, 3);
        }
    };
};